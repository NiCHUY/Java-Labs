    ...
    public static void main(String[] args) throws InputException {
        ...
        for (int index = 0; index < indexes1.size(); index++){
            ...
            for(int i = 0; i < indexes1.size(); i++){
                if (indexes1.get(i) < indexes2.get(0)) {
                    count++;
                    indexNew++;
                }
            }
            ...
            if (count / 2 > 0){
                string.delete(indexes1.get(0), indexes1.get(1) + 1);
                int diff = indexes1.get(1) - indexes1.get(0)+1;
                for(int i = 0; i < indexes1.size(); i++){
                    indexes1.set(i, indexes1.get(i) - diff);
                    indexes2.set(i, indexes2.get(i) - diff);
                }
                string.delete(indexes2.get(count-2), indexes2.get(count-1) + 1);
                diff = indexes2.get(count-1) - indexes2.get(count-2) + 1;
                for(int i = count; i < indexes1.size(); i++){
                    indexes1.set(i, indexes1.get(i) - diff);
                    indexes2.set(i, indexes2.get(i) - diff);
                }
                indexes1.remove(0);
                indexes2.remove(count - 2);
                indexes1.remove(0);
                indexes2.remove(count - 2);
                index = 0;
            }
